...
ggplot(data = acc1)+stat_count(mapping = aes(x= HOUR))+ theme_bw()
```
...
accidentData %>% group_by(COUNTY, STATE) %>% summarise(number_of_case = nrow())
...
```{r}
accState <- accidentData %>% inner_join(per, by = c('ST_CASE', 'STATE', 'COUNTY'))
